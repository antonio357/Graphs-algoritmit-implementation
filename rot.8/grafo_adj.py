...
class Grafo:
    ...
    def adiciona_aresta(self, a):
        ...
        if self.aresta_valida(a[0]) and self.peso_valido(peso):
            ...
            a = a[0].split(Grafo.SEPARADOR_ARESTA)  # evita um bug de quando a String da arestas tem um len() > 3
            # print("COM O SPLIT")
            # print("a = {}".format(a))
            # print("a[0] = {}".format(a[0]))
            # print("a[-1] = {}".format(a[-1]))
            # para grafo não direcionado
            if a[0] in self.VA.keys():
                # if aresta not in self.VA[a[0]]:
                self.VA[a[0]].append(aresta)
            else:
                self.VA[a[0]] = [aresta]
            if a[-1] in self.VA.keys():
                # if aresta not in self.VA[a[0][-1]]:
                self.VA[a[-1]].append(aresta)
            else:
                self.VA[a[-1]] = [aresta]

            # para grafo direcionado
            if a[0] in self.VAD.keys():
                # if aresta not in self.VAD[a[0]]:
                self.VAD[a[0]].append(aresta)
            else:
                self.VAD[a[0]] = [aresta]
    ...
    def Khan(self):
        ...
        fontes = []
        # ou seja vetices que não tem arestas
        for v in self.N:
            if v not in ditND.keys():
                OrdTopo.append(v)
    ...
    def DFS(self):
        ...
        # ou seja vetices que não tem arestas
        for v in self.N:
            if v not in self.VA.keys():
                verticesPercorridos.append(v)
        # print("\nantes da recursão")
        # print("verticesPercorridos = {}".format(verticesPercorridos))
        while len(verticesPercorridos) < len(self.N):
            raiz = self.raiz(verticesPercorridos)
            self.recursion(raiz, verticesPercorridos, arestasPercorridas, OrdTopo)
        verticesPercorridos.reverse()
        return verticesPercorridos

    def raiz(self, verticesPercorridos):
        for v in sorted(self.N):
            if v not in verticesPercorridos:
                return v
        pass

    def nextV(self, ED, verticeAtual):
        # print("\nnextV")
        # print("verticeAtual = {}".format(verticeAtual))
        strV = self.MD[ED][0]
        strV = strV.split(Grafo.SEPARADOR_ARESTA)
        # print("strV = {}".format(strV))
        if strV[0] == verticeAtual:
            # print("return = {}".format(strV[-1]))
            return strV[-1]
        elif strV[-1] == verticeAtual:
            # print("return = {}".format(strV[0]))
            return strV[0]

    def recursion(self, verticeAtual, verticesPercorridos, arestasPercorridas, OrdTopo):
        """
        percorre o grafo de forma recursiva manipulando os argumentos pelo o alg. da DFS
        para criar uma Ordenação Topologica
        :param verticeAtual: vertices de origim da recursão
        :param verticesPercorridos: quarda todos os vertices que a recursão já percorreu
        :param arestasPercorridas: quarda todas as arestas que a recursão já percorreu
        :param OrdTopo: faz Ordenação Topologica do Grafo
        :return:
        """
        # print("\nrecursion")
        # print("verticePercorridos = {}".format(verticesPercorridos))
        if verticeAtual in self.VAD.keys():
            for ed in self.VAD[verticeAtual]:
                proximoVertice = self.nextV(ed, verticeAtual)
                if ed not in arestasPercorridas and proximoVertice not in verticesPercorridos:
                    arestasPercorridas.append(ed)
                    self.recursion(proximoVertice, verticesPercorridos, arestasPercorridas, OrdTopo)
        # if verticeAtual not in verticesPercorridos:
        verticesPercorridos.append(verticeAtual)
